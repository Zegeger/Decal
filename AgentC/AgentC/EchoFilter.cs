...
{
    public delegate void KillMessageEvent(KillMessageEventArgs e);

    public class KillMessageEventArgs
    {
        public string KillMessage { get; private set; }

        internal KillMessageEventArgs(string killMessage)
        {
    ...
    class EchoFilter
    {
        ...
        public KillMessageEvent KillMessage;
                ...
                case GAME_EVENT:
                    ...
                    switch (gameEvent)
                    {
                        case KILL_DEATH_MESSAGE:
                            string killmessage = e.Message.Value<string>("text");
                            TraceLogger.Write("Kill Message: " + killmessage, TraceLevel.Verbose);
                            if (KillMessage != null)
                            {
                                TraceLogger.Write("Raising KillMessageEvent", TraceLevel.Verbose);
                                KillMessage(new KillMessageEventArgs(killmessage));
                            }
                            break;
                    }
                    break;
